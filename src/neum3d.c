...
RUN_HEAD(neum3d)
{
  ...
  for(v=v0;v<=v1;v++) for(z=s.z0;z<=s.z1;z++) {
    for(x=s.x0;x<=s.x1;x++) {
      y=s.y0; u=New+ind(x,y,z,v); u[-DY]=u[0]; /* bottom */
      y=s.y1; u=New+ind(x,y,z,v); u[+DY]=u[0]; /* top */
    }
    /* limits extended by 1 to include corner cells */
    for(y=s.y0-1;y<=s.y1+1;y++) {
      x=s.x0; u=New+ind(x,y,z,v); u[-DX]=u[0]; /* left */
      x=s.x1; u=New+ind(x,y,z,v); u[+DX]=u[0]; /* right */
    }
  }

  /* limits extended by 1 to include corner cells */
  for(v=v0;v<=v1;v++) for(y=s.y0-1;y<=s.y1+1;y++) for(x=s.x0-1;x<=s.x1+1;x++) {
    New[ind(x,y,s.z0-1,v)] = New[ind(x,y,s.z0,v)];
    New[ind(x,y,s.z1+1,v)] = New[ind(x,y,s.z1,v)];
  }
