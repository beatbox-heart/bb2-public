...
typedef struct {
  ...
  int g;			/* layer whereto to import the colour components */
  real g0,g1;			/* range of values to transfrom node to a byte for the grey level. */
} STR;
...
RUN_HEAD(pgmin) {
  ...
  for (z=s.z0;z<=s.z1;z++) {
    for (y=s.y0;y<=s.y1;y++) { 
      for (x=s.x0;x<=s.x1;x++) {
	if (1!=fread(inbuf,sizeof(*inbuf),1,f)) EXPECTED_ERROR("bad read byte number %ld from %s\n",i,filename);
	New[ind(x,y,z,g)]=unbyte(*inbuf,max,g0,g1);
	i++;
      } /*  for x */
    } /*  for y */
  } /*  for z */
  MESSAGE("pgmin from '%s' at t=%ld\n",filename,t);
  fclose(f);
  return 1;
} RUN_TAIL(pgmin)
