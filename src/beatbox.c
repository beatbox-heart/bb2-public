...
/* Static variables */
static char HEADLINE[MAXSTRLEN];	/* First line of output with package version variation and date */
...
int main (int argc, char **argv)
{
  ...
  /* Profiling stuff */
  int *timesCalled=NULL;/* number of times each device was executed */
  double spent;		/* time spent for a device call */
  double *totalSpent=NULL; /* total time spent by each device */
  ...
  for (iarg=1;iarg<argc;iarg++) {
	...
	if (0==strcmp(strcpy(debugname,argv[iarg]),"stdout"))
	  ...
	  snprintf(longdebugname,MAXPATH,debugname,mpi_rank);
  ...
  if (!qopen(argv[1])) EXPECTED_ERROR("Cannot open input file %s\n", argv[1]);
  ...
  *logname='\0';
  ...
  for (;;) { 
	...
	if (d.s.relist) if (*(d.s.relist)) list_space(&(d.s));
	/*
