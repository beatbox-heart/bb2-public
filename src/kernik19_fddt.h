...
(*i_CaL_Ca) = (((ibarca*d)*f)*fCa); /* && (mV/ms) */
(*i_CaL_K) = (((ibark*d)*f)*fCa); /* && (mV/ms) */
(*i_CaL_Na) = (((ibarna*d)*f)*fCa); /* && (mV/ms) */
...
/* Jleak */ 
(*j_leak) = ((Ca_SR - Cai)*V_leak); /* && (mM/ms) */

/* INaCa */
(*i_NaCa) = (kNaCa*(((exp(((gamma*V)*FRT))*cub(Nai)*Cao) - (((exp((((gamma - 1.0)*V)*FRT))*cub(Nao)*Cai)*alpha)))/(((cub(KmNai)+cub(Nao))*(KmCa+Cao))*(1.0+(Ksat*exp((((gamma - 1.0)*V)*FRT))))))); /* && (mV/ms) */ 

/* INak */
(*i_NaK) = (((PNaK*Ko)*Nai)/(((Ko+Km_K)*(Nai+Km_Na))*((1.0+(0.1245*exp(((-0.1*V)*FRT))))+(0.0353*exp((-V*FRT)))))); /* && (mV/ms) */

/* IpCa */
(*i_PCa) = ((g_PCa*Cai)/(Cai+KPCa)); 

/* Ito */
(*i_to) = ((g_to*r*s)*(V - (*E_K))); /* && (mV/ms) */

/* ICaT */ 
(*i_CaT) = (((g_CaT*d_icat)*f_icat)*(V - (*E_Ca))); /* && (mV/ms) */

/* ifunny */
(*i_fNa) = (((Na_frac*g_f)*Xf)*(V - (*E_Na))); /* && (mV/ms) */
(*i_fK) = ((((1.0 - Na_frac)*g_f)*Xf)*(V - (*E_K))); /* && (mV/ms) */
(*i_f) = ((*i_fNa)+(*i_fK)); /* && (mV/ms) */

/* IK1 */
(*i_K1) = (((g_K1*sqrt((Ko/5.4)))*XK1_inf)*(V - (*E_K))); /* && (mV/ms) */

/* IKr */
(*i_Kr) = ((((g_Kr*sqrt((Ko/5.4)))*Xr1)*Xr2)*(V - (*E_K))); /* && (mV/ms) */

/* IKs */
(*i_Ks) = ((g_Ks*sqr(Xs))*(V - (*E_K))); /* && (mV/ms) */
...
(*diff_I) = (kiSRCa*Cai)*O - (kim)*I - (kom)*I + (koSRCa*sqr(Cai))*RI; /* && (1/ms) */
(*diff_O) = (koSRCa*sqr(Cai))*R1 - (kom)*O - (kiSRCa*Cai)*O + (kim)*I; /* && (1/ms) */
(*diff_R1) = (kim)*RI - (kiSRCa*Cai)*R1 - (koSRCa*sqr(Cai))*R1 + (kom)*O; /* && (1/ms) */

(*j_rel) = ((((ks*O)*(Ca_SR - Cai))*(V_SR))/Vc); /* && (mM/ms) */


/* Jup */
(*j_up) = (VmaxUp/(1.0+(sqr(Kup)/sqr(Cai)))); /* && (mM/ms) */

/* Nai */
(*diff_Nai) = ((-Cm/(F*Vc))*(((((((*i_Na))+(*i_b_Na))+(*i_fNa))+(3.0*(*i_NaK)))+(3.0*(*i_NaCa)))+(*i_CaL_Na))); /* && (mM/ms) */

/* Cai */
real Cai_bufc = (1.0/(1.0+((Buf_C*Kbuf_C)/((Cai+Kbuf_C)*(Cai+Kbuf_C))))); /* && */
(*diff_Cai) = (Cai_bufc*(((-(*j_up)+(*j_leak))+(*j_rel)) - ((Cm/((2.0*Vc)*F))*((((((*i_CaL_Ca))+(*i_CaT))+(*i_b_Ca))+(*i_PCa)) - (2.0*(*i_NaCa)))))); /* && (mM/ms) */

/* Ki */
(*diff_Ki) = ((-Cm/(F*Vc))*((((*i_K1)+(*i_to)+(*i_Kr)+(*i_Ks)+(*i_fK)+(*i_KACh)) - (2.0*(*i_NaK)))+(*i_CaL_K))); /* && (mM/ms) */

/* CaSR */
(*Ca_SR_bufSR) = (1.0/(1.0+((Buf_SR*Kbuf_SR)/sqr(Ca_SR+Kbuf_SR)))); /* && */
(*diff_Ca_SR) = ((((*Ca_SR_bufSR)*Vc)/V_SR)*(((*j_up) - (*j_rel)) - (*j_leak))); /* && (mM/ms) */


/* V */
(*diff_V) = -((*i_K1)+(*i_to)+(*i_Kr)+(*i_Ks)+(*i_KACh)+(*i_CaL)+(*i_CaT)+(*i_NaK)+(*i_Na)+(*i_NaCa)+(*i_PCa)+(*i_f)+(*i_b_Na)+(*i_b_Ca));	/* (mV/ms) */
