...
/*
  ...
  (same as ppcpash18.c now renamed for easier mnenomic)
  ...
  NB all essential contents are stoved away into paci18_*.h files. 
*/
...
#include "paci18_const.h"

/* Enumerate all dynamic variables */
enum {
  #define _(n,i) var_##n,
  #include "paci18_other.h"
  #include "paci18_ngate.h"
  #include "paci18_tgate.h"
  #undef _
...
IONIC_FDDT_HEAD(paci18,NV,NTAB,NO,NN) {
...
int create_paci18rhs (Par *par, Var *var, char *w, real **u, int v0)
{
  /* printf("xxxxxxxxxxxxxxxx create_paci18rhs entered\n"); */
  STR *S = (STR *)Calloc(1,sizeof(STR));
  char *ptr=w;
  int ivar=0;
  if (!S) ABORT("cannot create %s","paci18rhs");
